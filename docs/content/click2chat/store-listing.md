---
shortDescription: View recent calls and start WhatsApp chats without saving contacts.
---

Click2Chat helps you continue a conversation without first adding a phone number to your contacts.

Start a Direct Chat

• Choose a country or enter a custom calling code
• Enter or paste a mobile number
• Add an optional message
• Open the conversation in WhatsApp or WhatsApp Business
• Keep the selected country and calling code synchronized

If both WhatsApp applications are installed, Click2Chat lets you choose which one to use. If only one is available, the Android app can open it directly.

Recent Calls on Android

The Android build uses android.permission.READ_CALL_LOG to display recent call history after you grant Android's runtime permission.

• Review up to 100 recent calls
• See cached caller names when Android provides them
• Distinguish incoming, outgoing, missed, rejected, blocked, and voicemail calls
• Review call time and duration when available
• Start a WhatsApp chat from the message button beside a call

Recent calls are held temporarily while the screen is in use. Click2Chat does not save the retrieved call list to an app database, backup, or developer server. Direct-number chat remains available if call-log permission is denied or unavailable.

Privacy by Design

• No account or sign-in
• No advertisements
• No analytics or tracking
• No developer-operated cloud storage
• No contact saving or number tagging
• No SMS, Contacts, microphone, location, or broad storage permission
• Clipboard access occurs only when you press paste

Click2Chat stores only the selected country and appearance preference locally. Phone numbers and optional messages are passed to WhatsApp only after you request the action.

Appearance

Choose light mode, dark mode, or follow your device's system theme. The Android status and navigation bars follow the selected appearance.
